...
fab_html = """
    ...
    <div id="fab-menu" style="position: fixed; bottom: 25px; right: 25px; z-index: 9999; text-align: right;">
        <div id="fab-links" style="display: none; flex-direction: column; gap: 8px; margin-bottom: 15px; background: rgba(255,255,255,0.95); padding: 12px; border-radius: 12px; box-shadow: 0 6px 16px rgba(0,0,0,0.25); border: 2px solid var(--vcv-dorado); text-align: left; backdrop-filter: blur(5px);">
            ...
            <a href="#calendarioSection" onclick="toggleFab()" style="color: var(--vcv-morado); font-weight: bold; font-size: 1.1rem; text-decoration: none; padding: 8px 10px; display: block; border-bottom: 1px solid #ddd;">🏐 Próximos Partidos</a>
            ...
            <a href="#prediccionesTotalesSection" onclick="toggleFab()" style="color: var(--vcv-morado); font-weight: bold; font-size: 1.1rem; text-decoration: none; padding: 8px 10px; display: block;">🔮 Predicción Final</a>
        </div>
...
if 'fab-menu' not in html:
    html = html.replace('<div id="appSection" style="display: none;">', '<div id="appSection" style="display: none;">\n' + fab_html)
    ...
    with open('dev.html', 'w', encoding='utf-8') as f:
        f.write(html)
    print("FAB menu injected in dev.html successfully.")
else:
