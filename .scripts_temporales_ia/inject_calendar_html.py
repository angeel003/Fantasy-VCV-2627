...
# Let's insert it before `<div class="modal fade" id="infoModal"`
# Or before `<footer`
# Actually, it belongs inside `appSection` so it's hidden when logged out.
# Find `</div>\n    \n    <!-- MODAL BOOTSTRAP -->`
idx = html.find('<!-- MODAL BOOTSTRAP -->')
if idx != -1:
    # Go backwards to find the closing div of appSection
    end_app_section = html.rfind('</div>', 0, idx)
    if end_app_section != -1:
        if 'id="calendarioSection"' not in html:
            html = html[:end_app_section] + calendar_html + "\n" + html[end_app_section:]
            print("Injected HTML")
else:
    print("Could not find MODAL BOOTSTRAP")
