...
for filename in files:
    ...
    # The existing block
    old_onedit = """  else if (sheet.getName() === "Usuarios") {
    if (col === 1 && row > 1) {
      if (e.oldValue && e.value && e.oldValue !== e.value) {
        ...
        clearAllCache();
          ss.toast("Se ha actualizado el usuario '" + usuarioAntiguo + "' a '" + usuarioNuevo + "' en todas las pestaas.", " Actualizacin Mgica", 5);
      }
    }
  }"""
    ...
    if old_onedit in js:
        js = js.replace(old_onedit, new_onedit)
