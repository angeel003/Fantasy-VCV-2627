...
# 2. Re-write Login Section
login_start = text.find('<div id="loginSection">')
# the login section ends where? Let's just find the next <!-- LOADER -->
login_end = text.find('<div id="appSection"', login_start)

if login_start != -1 and login_end != -1:
    new_login_html = """<div id="loginSection" style="display:flex; justify-content:center; align-items:center; background:var(--bg-card-alt); padding:40px 20px;">
...
"""
    # Replace the old loginSection block
    text = text[:login_start] + new_login_html + text[login_end:]
...
# 4. HTML Block for Match Card (Same as before)
...
# 5. Fix `cargarDatosAntiguos` exactly
start_load_func = text.find('function cargarDatosAntiguos() {')
if start_load_func != -1:
    # Find the specific line: for(var idPart in preds) {
    loop_start = text.find('for(var idPart in preds) {', start_load_func)
    loop_end = text.find('}', loop_start) + 1 # wait, the loop has multiple lines.
    # It looks like:
    ...
    # We find the specific block and inject the UI sync after the setVal's
    old_loop_block = text[loop_start:text.find('}', text.find('setVal', loop_start))+1]
    
    new_loop_block = """for(var idPart in preds) {
                setVal(`e${idPart}_sets`, preds[idPart].sets);
                setVal(`e${idPart}_puntos`, preds[idPart].puntos);
                setVal(`e${idPart}_signo`, preds[idPart].signo);
    ...
    text = text[:loop_start] + new_loop_block + text[text.find('}', text.find('setVal', loop_start))+1:]
