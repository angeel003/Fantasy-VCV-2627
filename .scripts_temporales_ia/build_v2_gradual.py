...
# CSS to inject in <head> for the new cards
new_css = """
...
# New interactive JS functions to inject before closing </body>
interactive_js = """
<script>
    ...
    function seleccionarSetV2(btn, val, matchId) {
      ...
      if (hiddenInput) {
          // If already selected, maybe unselect? No, let's keep it simple.
    ...
    function cambiarPuntosV2(matchId, delta) {
      ...
      if (hiddenInput && valSpan) {
          let current = parseInt(hiddenInput.value) || 14; // default if empty
          let next = current + delta;
          if (next < 1) next = 1; // don't go below 1
    ...
    function cambiarSignoV2(matchId, btn, val) {
      ...
      if (hiddenInput) {
          // Convert visual sign to actual value expected by backend
          // The backend expects "A favor" or "En contra" exactly based on old select options
...
# Replace the HTML literal string for ABIERTO matches
# Find the start of the ABIERTO block
old_card_pattern = r'if \(eq\.estado === "ABIERTO"\) \{.*?</select>\s*</div>\s*</div>\s*\$\{destacadoHtml\}\s*</div>`;'
old_card_block = re.search(old_card_pattern, text, re.DOTALL)
...
if old_card_block:
    new_card_html = r"""if (eq.estado === "ABIERTO") {
                        ...
                        // Ensure we initialize Lucide icons after rendering
                        ...
                        <div class="vcv-card-v2" data-category="${eq.categoria}">
                            ...
                            <!-- HIDDEN INPUTS TO PRESERVE SAVE LOGIC -->
    ...
    text = text.replace(old_card_block.group(0), new_card_html)
else:
    print("WARNING: Could not find ABIERTO block to replace")

# We also need to update the `cargarDatosAntiguos()` so it updates the new UI properly
# Original logic: setVal(`e${idPart}_sets`, preds[idPart].sets);
# Since we use hidden inputs, the value is updated, but the UI is NOT updated automatically.
# We must hook into it.
fix_cargar_datos = """
        if(data.status === "success") {
            ...
            for(var idPart in preds) {
                ...
                if (preds[idPart].signo) {
                    const signoVal = preds[idPart].signo; // "A favor" o "En contra"
                    if (signoVal === "A favor") {
...
"""
text = re.sub(r'if\(data\.status === "success"\) \{.*?for\(var idPart in preds\) \{.*?\}\s*\}', fix_cargar_datos, text, flags=re.DOTALL)


with open('v2.html', 'w', encoding='utf-8') as f:
    f.write(text)

print("v2.html created by cloning index.html and gradually migrating Cartelera!")
