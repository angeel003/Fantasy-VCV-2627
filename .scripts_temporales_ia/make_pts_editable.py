...
new_js = r"""function sincronizarPuntosV2(matchId) {
      ...
      if (hiddenInput && valInput) {
          ...
          // We don't force the input value to 0 immediately if they are deleting to retype,
          // but we do update the hidden input and state.
...
# Before modifying, check if script already ran
if 'sincronizarPuntosV2' not in text:
    text = re.sub(old_js, new_js, text)
    text = re.sub(old_logic, new_logic, text, flags=re.DOTALL)
    
    # We must also change valSpan.innerText = next; to valInput.value = next; (or support both)
